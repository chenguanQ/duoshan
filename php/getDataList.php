...
{
  # 获取所有的商品信息
  $sql = "SELECT * FROM `goodslistdata` order by `gid` limit $page , 20";
}else if($typeOrder == 3)
{
  $sql = "SELECT * FROM `goodslistdata` ORDER BY `goodslistdata`.`price` DESC limit $page , 20";
}else if($typeOrder == 3)
{
  $sql = "SELECT * FROM `goodslistdata` ORDER BY `goodslistdata`.`price` ASC limit $page , 20";
}
